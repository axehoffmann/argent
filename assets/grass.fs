...
void main()
{
	...
	vec3 N = normal; // * (2 * int((gl_FrontFacing && normal.y < 0) || (!gl_FrontFacing && normal.y > 0)) - 1);
	N = abs(normalize(N));

	vec3 colour = mix(vec3(0.0, 0.4, 0.7), vec3(0.0, 0.8, 0.4), posAlongBlade); 
	...
	fragColour = vec4(diffuse + spec + vec3(0.3) * colour, 1.0);
	//N *= 0.5;
	//N += 0.5;
